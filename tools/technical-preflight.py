...
EXPECTED_CACHE_VERSION = "179"
EXPECTED_CASE_CACHE_VERSION = "179"
...
def scan() -> list[str]:
    ...
    pages = sitemap_pages()
    if len(pages) != 164:
        findings.append(f"sitemap.xml: expected 164 URLs, found {len(pages)}")
...
def main() -> int:
    ...
    if findings:
        ...
        for finding in findings:
            print(f"- {finding}")
        return 1
    print(f"Technical preflight: {len(sitemap_pages())} pages, 0 findings")
    return 0
